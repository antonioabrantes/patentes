...
<html lang="pt-br">
	...
	?>
		...
		<h1>Pedidos pendentes em Recursos de Patentes (COREP) </h1>
		...
		<table class="table table-hover align-middle table-status table-striped">
			...
			<thead>
			<tr width=80%>
				<th width=6%>Divisão</th>
				<th width=6%>Ano pendente</th>
...
<?php
		...
		foreach ($divisoes2 as $idivisao)
		{
			...
			$ano_mais_antigo[$idivisao] = 2026;
			$qtd_mais_antigo[$idivisao] = 0;
		}
		$array_numero = array();
		$array_data = array();
		$array_divisao = array();
						...
						else
						{
							...
							else
							{
								...
								else
								{
									...
									$iano = (int)substr($data12,0,4);
									if ($iano < $ano_mais_antigo[$idivisao]) 
									{
										$ano_mais_antigo[$idivisao] = $iano;
										$qtd_mais_antigo[$idivisao] = 0;
									}
									if ($iano == $ano_mais_antigo[$idivisao]) $qtd_mais_antigo[$idivisao]++;
									
									if ($iano < $ano_mais_antigo['dirpa']) 
									{
										$ano_mais_antigo['dirpa'] = $iano;
										$qtd_mais_antigo['dirpa'] = 0;
									}
									if ($iano == $ano_mais_antigo['dirpa']) $qtd_mais_antigo['dirpa']++;
									$ii++;
								}
							}
						}
		...
		{
			echo "<tr>";
			$setor = $divisao_complemento[$idivisao];
			if ($idivisao=='dirpa')
				echo "<td style='font-size: 12px;'>DIRPA</td>";
			else
				echo "<td style='font-size: 12px;'><a href='infopedidos_pendentes.php?setor=$setor'>$setor</a></td>";
			echo "<td style='font-size: 12px;'>$ano_mais_antigo[$idivisao]</td>";
			echo "<td style='font-size: 12px;'>$qtd_mais_antigo[$idivisao]</td>";
			echo "</tr>";
			$str = "$numero;$ano[$divisao];$quantidade[$pendente]"."\n";
			fputs($fp,$str);
			echo "</tr>";
		}
